...
-- manager v1 adds an approval gate.
--
-- DEFAULT is 'auto' (not 'pending') so that:
--   1. Every PRE-EXISTING ig_posts row added before this migration
--      gets approval_status='auto' automatically when the column is
--      added — no backfill UPDATE needed, no risk of stalling already
--      scheduled posts behind the new gate.
--   2. Any LEGACY code path that inserts an ig_post (the existing
--      cadence/publish flow) continues to behave the way it always
--      has — drafts auto-publish at scheduled_at.
--   3. Phase C's NEW cadence engine will EXPLICITLY pass
--      approval_status='pending' on every insert, opting those rows
--      into the human-review gate. The new approval queue UI flips
--      them to 'approved' before the publish cron picks them up.
--
-- This is strictly safer than a 'pending' default + backfill — the
-- earlier approach broke on schemas where ig_posts.created_at doesn't
-- exist, and was vulnerable to silent state flipping on re-run.
alter table public.ig_posts
  ...
  add column if not exists approval_status text default 'auto' check (
