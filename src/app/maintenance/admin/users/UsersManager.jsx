...
export function UsersManager({ initialUsers, currentUserId }) {
  ...
  async function submitForm(e) {
    ...
    setBusy(true);
    try {
      const r = await fetch('/api/maintenance/admin/users', {
        method: 'POST',
  ...
  async function resetPassword(user) {
    ...
    setFlash('');
    try {
      const r = await fetch(`/api/maintenance/admin/users/${user.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function revokeAccess(user) {
    ...
    setError('');
    try {
      const r = await fetch(`/api/maintenance/admin/users/${user.id}`, { method: 'DELETE' });
      const json = await r.json();
    ...
    <div className="mt-8 space-y-12">
      ...
      <section>
        ...
        <p className="mt-3 text-xs text-brand-slate">
          ...
          <a className="underline" href="/maintenance/admin/login">/maintenance/admin/login</a>
        </p>
      </section>
