...
export const metadata = { title: 'Marketing · Ads', robots: { index: false, follow: false } };
export const dynamic = 'force-dynamic';

export default async function AdsPage({ params }) {
  const auth = await isOwner();
  const p = params instanceof Promise ? await params : params;
  if (!auth.authed) redirect(`/admin/login?redirect=/admin/marketing/${p.property}/ads`);
      ...
      <main className="animate-page-in bg-brand-cloud pt-24 lg:pt-28">
        <div className="mx-auto flex max-w-[88rem]">
          ...
          <div className="min-w-0 flex-1 px-5 py-10 sm:px-8 lg:px-10">
            ...
            <PhaseStub
              phase="D"
              title="Paid ads roll-up"
              description="Read-only dashboard combining Meta Marketing API + Google Ads API: spend, impressions, clicks, conversions, ROAS — all attributed back to per-property bookings via the UTM tagging layer."
