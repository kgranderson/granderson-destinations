 ...
 *
 * All mutations go through the /api/admin/vendors* endpoints
...
export function VendorsManager({ initialVendors, categories, properties }) {
  ...
  async function toggleActive(vendor) {
    setError('');
    try {
      const r = await fetch(`/api/admin/vendors/${vendor.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      fd.append('file', file);
      const r = await fetch('/api/admin/vendors/bulk', { method: 'POST', body: fd });
      const json = await r.json();
      if (!r.ok && r.status !== 400) throw new Error(json.error || 'Upload failed');
      setUploadSummary(json);
      // Re-fetch list
      const listRes = await fetch('/api/admin/vendors');
    ...
    <div className="mt-8 space-y-12">
      ...
      <section className="rounded-xl border border-brand-slate/15 bg-white p-6">
        ...
        <p className="mt-3 text-sm text-brand-slate">
          Need the template?{' '}
          <a
            href="/api/admin/vendors/template"
